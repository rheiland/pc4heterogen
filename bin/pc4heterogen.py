...
def write_config_file(name):
    ...
    # print("pc4heterogen.py: ------- sub.numx, sub.numy = ", sub.numx, sub.numy)
...
def run_done_func(s, rdir):
    ...
    if nanoHUB_flag:
        ...
        os.system("submit  mail2self -s 'nanoHUB pc4heterogen' -t 'Your Run completed.'&")
...
def run_sim_func(s):
    ...
    if nanoHUB_flag:
        if remote_cb.value:
            s.run(run_name, "-v ncn-hub_M@brown -n 8 -w 1440 pc4heterogen-r7 config.xml")   # "-r7" suffix??
        else:
